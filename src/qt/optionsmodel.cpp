...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BLUE);
    ...
    if (!settings.contains("nAnonymizeBluecoinAmount"))
        settings.setValue("nAnonymizeBluecoinAmount", 1000);

    nAnonymizeBluecoinAmount = settings.value("nAnonymizeBluecoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBluecoinAmount"))
        SoftSetArg("-anonymizebluecoinamount", settings.value("nAnonymizeBluecoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bluecoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBluecoinAmount:
            return QVariant(nAnonymizeBluecoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBluecoinAmount:
            nAnonymizeBluecoinAmount = value.toInt();
            settings.setValue("nAnonymizeBluecoinAmount", nAnonymizeBluecoinAmount);
            emit anonymizeBluecoinAmountChanged(nAnonymizeBluecoinAmount);
            break;
        case CoinControlFeatures:
