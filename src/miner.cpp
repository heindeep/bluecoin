        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    // zBLUE spends can have very large priority, use non-overflowing safe functions
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BluecoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("BluecoinMiner : ProcessNewBlock, block not accepted");
...
void BitcoinMiner(CWallet* pwallet, bool fProofOfStake)
{
    ...
    LogPrintf("BluecoinMiner started with fProofOfStake=%d\n", fProofOfStake);
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running BluecoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
