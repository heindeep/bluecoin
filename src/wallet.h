...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 BLUE at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBLUE send
enum ZerocoinSpendStatus {
    ZBLUE_SPEND_OKAY = 0,                            // No error
    ZBLUE_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBLUE_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBLUE_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBLUE_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBLUE_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBLUE_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBLUE_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBLUE_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBLUE_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBLUE_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBLUE_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBLUE_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBLUE_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBLUE_SPENT_USED_ZBLUE = 14,                      // Coin has already been spend
    ZBLUE_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
public:
    ...
    */
    ...
    /** zBLUE reset */
    boost::signals2::signal<void()> NotifyzBLUEReset;
};
